...
def countNode(update, context):
    ...
    if is_gdrive_link(link):
        msg = sendMessage(f"¢συитιиg g∂яινє ℓιик : <code>{link}</code>", context.bot, update)
        ...
        if uname is not None:
            cc = f'\n\n<b>𝘾𝙤𝙪𝙣𝙩𝙞𝙣𝙜 𝙗𝙮: </b>{uname}'
        sendMessage(result + cc, context.bot, update)
        if gdtot_link:
            gd.deletefile(link)
    else:
        sendMessage('sᴇɴᴅ ɢᴅʀɪᴠᴇ ᴀʟᴏɴɢ ᴡɪᴛʜ ᴄᴏᴍᴍᴀɴᴅ ᴏʀ ʙʏ ʀᴇᴘʟʏɪɴɢ ᴛᴏ ᴛʜᴇ ʟɪɴᴋ ʙʏ ᴄᴏᴍᴍᴀɴᴅ', context.bot, update)
